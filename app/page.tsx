...
function isAiEducationTrend(item: EducationNews) {
  const category = inferNewsCategory(item.title, item.category);
  const text = [item.title, item.translated_title, item.category, item.source, item.summary, item.teacher_insight, item.school_action]
    .filter(Boolean)
    .join(" ");

  return category === "AI교육" || aiEducationPattern.test(text);
}
...
export default async function Home({
  ...
  const majorNews = news
    ...
    .slice(0, 5);
  const { kept: aiTrendNews } = dedupeNewsForDisplay(news.filter(isAiEducationTrend));
  const highlightedAiTrendNews = aiTrendNews.slice(0, 4);
